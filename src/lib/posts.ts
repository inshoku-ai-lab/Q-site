...
// Slug lookup restricted to what the site actually publishes -- used by
// the member-content API so an unpublished post can't be pulled through
// it by slug alone.
...
// Only http(s) URLs are ever turned into a link, a card or an embed.
// Article HTML is machine-generated from Notion (all text escaped), but
// the URLs inside it are a decade of migrated WordPress content, so a
// `javascript:`/`data:` URL that survived migration must not become a
// live href.
...
// Schemes allowed to survive in an <a href> that came through migration.
// Anything else that *looks* like a scheme (javascript:, data:, vbscript:)
// is defused rather than dropped, so the link text stays readable.
const ALLOWED_LINK_SCHEMES = new Set(["http:", "https:", "mailto:", "tel:"]);
const HAS_SCHEME_RE = /^[a-z][a-z0-9+.-]*:/i;

// Article HTML is generated by scripts/sync-notion.mjs, which escapes all
// text and only emits a fixed set of tags -- the one attacker-reachable
// value left in it is the href, which is copied verbatim from a decade of
// migrated WordPress content. Relative links are left alone; only an
// explicit, non-web scheme is rewritten.
function sanitizeHrefs(html: string): string {
  return html.replace(/href="([^"]*)"/gi, (match, raw: string) => {
    // Control characters and entity-encoded ampersands are stripped first
    // so "java\tscript:..." can't slip past the scheme test.
    const value = raw.replace(/&amp;/g, "&").replace(/[\u0000-\u0020]/g, "").trim();
    if (!HAS_SCHEME_RE.test(value)) return match;
    try {
      return ALLOWED_LINK_SCHEMES.has(new URL(value).protocol) ? match : 'href="#"';
    } catch {
      return 'href="#"';
    }
  });
...
// An <img>/<iframe> source is renderable if it is a same-site absolute
// path (migrated WordPress media lives under /images/wp/) or an http(s)
// URL.
function isRenderableSrc(src: string): boolean {
  return src.startsWith("/") && !src.startsWith("//") ? true : isSafeHttpUrl(src);
}
...
export function preprocessBlocks(blocks: Block[]): Block[] {
  return blocks.flatMap((rawBlock) => {
    // sanitizeHrefs runs first, on the raw block, so every downstream
    // path (paragraph splitting, linkification, the member-content
    // renderer) sees markup whose links are already scheme-checked.
    const b: Block = {
      ...rawBlock,
      ...(rawBlock.html ? { html: sanitizeHrefs(applyContentFixups(unescapeMarkdown(rawBlock.html))) } : {}),
      ...(rawBlock.caption ? { caption: sanitizeHrefs(rawBlock.caption) } : {}),
    };
    ...
    if ((b.type === "bookmark" || b.type === "embed") && b.url) {
      return isValidUrl(b.url) ? [{ type: "blogcard", url: b.url }] : [];
    }
    // A video block becomes an <iframe src>, so an unusable or non-web
    // URL is dropped rather than embedded.
    if (b.type === "video") {
      return b.url && isValidUrl(b.url) ? [b] : [];
    }
    if (b.type === "image") {
      return b.src && isRenderableSrc(b.src) ? [b] : [];
    }
    if (b.html) {
...
// Wording that marks a callout as *the* free/member boundary rather than
// an ordinary aside. Matching one of these makes the intent explicit
// instead of inferring it from position alone.
const MEMBER_BOUNDARY_RE = /会員限定|ここから会員|旅路の証人|メンバー限定|🔒|member[\s-]?only/i;
...
//
// Historically *any* callout was the boundary, which meant dropping an
// ordinary note-callout into a free article silently paywalled everything
// after it. A callout whose text names the membership now wins; when no
// callout says so, the first one is still used, so every article written
// under the old rule keeps behaving exactly as before.
export function splitMemberContent(blocks: Block[]): {
  freeBlocks: Block[];
  memberBlocks: Block[];
  hasMemberContent: boolean;
} {
  const markedIdx = blocks.findIndex((b) => b.type === "callout" && MEMBER_BOUNDARY_RE.test(b.html ?? ""));
  const idx = markedIdx !== -1 ? markedIdx : blocks.findIndex((b) => b.type === "callout");
  if (idx === -1) {
