...
- 原文がすでに二重表記（「５０ルピー（１５０円）」）の場合は、**円を経由せず直接ドルへ**。
  ...
  ```
- **単位は「読者が何を思い浮かべる必要があるか」で決める。**
  - **物の寸法 → ヤード・ポンド法に換算する。** 米国読者は cm を体感できない。
    Ep 190 で「１５センチのチロム」を `fifteen centimeters` と訳したところ、
    **査読者3人全員が「換算しないと絵が浮かばない」と指摘**した。→ `about six inches long`
  - **距離・標高 → メートルのまま。** 数字の大小だけ伝わればよく、体感は要らない。
    体感が重要な箇所のみ `3,000 meters (about 10,000 feet)` と併記。
  - **重量 → 併記が無難。** `sixty kilos` は米国読者に伝わりにくい。
