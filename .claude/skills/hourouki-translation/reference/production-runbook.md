...
**バッチサイズ = 40話**（著者指示・2026-09-05 更新）。

経緯: 当初は「他作業とぶつかるので6話で固定」→ 別作業を他アカウントへ移したので
30話へ → さらに別セッションも移したので **40話** へ。

1バッチ = 40話 × 7工程 = **280エージェント**。残り約12バッチ。

> 通常のワークフロー規模の目安（15エージェント未満）を大きく超えるが、
> **著者が明示的にこの規模を要求している**。勝手に縮めない。

**ワークフローの利用は著者から恒久的に許可されている（2026-09-05）。**
起動のたびに確認を取らない。

**待ち時間は遊ばせない。** 翻訳ワークフローの実行中に、次のバッチぶんの
原文取得を並行して走らせる。取得エージェントは `episode-index.json` から
Notion ID を自分で読む（ID を人が書き写さない）。
