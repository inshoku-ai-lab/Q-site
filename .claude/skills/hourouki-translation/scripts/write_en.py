...
def strip_trailing_nav(lines):
    """前後記事ナビを落とす。**位置は末尾とは限らない。**

    Notionでの持ち方が2通りある。Ep 70以前は1行だが、**Ep 71以降は
    角括弧がエスケープされURLの途中で改行が入り、複数行に分かれる**：

        \\[前の記事０７０\\](https://.../070/
        )　｜　\\[次の記事０７２\\](https://.../072/
        )

    以前はこれを「最初のナビ行から末尾まで切る」で処理していた。
    ⚠️ **Ep 155 と Ep 182 は、記事の途中（callout の直後）にもナビ行を持つ。**
    末尾まで切る実装だと、本文がまるごと消える。実際に Ep 155 の照合元が
    35字まで削れて、会員限定マーカーの位置検証が誤ってERRORを出した。

    そこで、**ナビ行はどこにあっても、そこだけを落とす。**
    行頭がナビリンクの行から、リンクが閉じるまで（最大3行）を1組として捨てる。
    ...
    out, i = [], 0
    while i < len(lines):
        if not NAV_START.match(lines[i].strip()):
            out.append(lines[i])
            i += 1
            continue
        # リンクが閉じるまで（最大3行）まとめて捨てる。URL途中の改行に対応する
        j = i
