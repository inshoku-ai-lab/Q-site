...
excerpt: "ビットコイン歴史研究家カート・ワッカートJr氏のCJテレビでのインタビューを翻訳——ビルダーバーグ会議の会長やエプスタインがビットコイン開発陣営に多額の寄付をした資金の流れから、支配層がいかにして開発を乗っ取り、自分たちに都合のいいものに作り替えたかを解説する。クレイグ博士＝サトシを前提とした、ビットコイン裏面史の決定的な一本。"
flags: ""
---

みなさんこんにちは、メキシコの山奥で旅人たちと平和に暮らしているクリプトラベラーです。
...
（この仕組みのおかげで）マスターカードには常に小銭が流れ込むようになります。
...
最初はRedditから始まり、Bitcoin.orgがBitcoin Talkに移されたことなどの小さなことの積み重ねです。
...
そもそもグリッグが、ビットコインを可能にする技術を作りました。 それがトリプルエントリー会計であり、それがリカルディアン契約です。

彼は多くの古い会計原則をコンピュータ化し、コンピュータ科学に応用できるような数学に変えたのです。

だから、グリッグは数学の暗号の忍者のようなもので、現存する最高のコーダー（プログラマーのようなもの）であり、現存する最高の理論暗号解読者のようなものです。

彼はすごいんだ。

それで、サトシはグリッグに連絡を取って、「俺がサトシだってことを証明したいんだ。だって、誰も君には反論できないからね。」
