...
最終更新: 2026-09-05（Ep 259 まで着手した時点。訳了252本）
...
| 訳了 | **284本**（Ep 0〜279 の大半。`ls migration/posts-en | wc -l` が正） |
| 残り | 238本（Ep 280〜521 の大半。飛びがあるので上の gap 検査で確認する） |
...
20. **円→ドルは初出1回だけ。2つ目の金額には付けない。**
    Ep 103 に続いて Ep 262 でも再発（2万円と1万円の両方にドルを付けていた）。
    レートは一度教えれば読者が自分で割れる。
